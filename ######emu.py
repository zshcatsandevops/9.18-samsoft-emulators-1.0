...
class SamsoftEmuNES:
    def __init__(self, root):
        ...
        self.scale = 2
        self.running = False
        self.tv_system = 'NTSC'  # Default
        self.cycles_per_frame = 29780  # NTSC default
        self.fps = 60
        ...
        self.current_rom_path = None  # Track loaded ROM path
        self.setup_ui()
        self.load_config()
        self.load_rom()
    ...
    def load_rom(self):
        ...
        if rom_path:
            ...
            if self.nes.load_rom(rom_path):
                self.current_rom_path = rom_path  # Store path
                self.rom_loaded = True
                self.running = True
                # Detect region from filename
                rom_name = os.path.basename(rom_path).lower()
                pal_indicators = ['(e)', '(europe)', 'pal']
                if any(indicator in rom_name for indicator in pal_indicators):
                    self.tv_system = 'PAL'
                    self.cycles_per_frame = 33233  # PAL cycles per frame (~50 FPS)
                    self.fps = 50
                    self.frame_time = 1000 // self.fps
                    logging.info("Detected PAL ROM; adjusted to 50 FPS")
                else:
                    self.tv_system = 'NTSC'
                    self.cycles_per_frame = 29780  # NTSC cycles per frame (~60 FPS)
                    self.fps = 60
                    self.frame_time = 1000 // self.fps
                    logging.info("Detected NTSC ROM; adjusted to 60 FPS")
                status_text = f"ROM: {os.path.basename(rom_path)} ({self.tv_system})"
    ...
    def reset(self):
        ...
        if self.current_rom_path:
            self.nes.load_rom(self.current_rom_path)
            # Re-detect region for reset
            rom_name = os.path.basename(self.current_rom_path).lower()
            pal_indicators = ['(e)', '(europe)', 'pal']
            if any(indicator in rom_name for indicator in pal_indicators):
                self.tv_system = 'PAL'
                self.cycles_per_frame = 33233
                self.fps = 50
                self.frame_time = 1000 // self.fps
            else:
                self.tv_system = 'NTSC'
            ...
            logging.warning("No ROM path for reset")
